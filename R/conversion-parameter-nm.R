...
setMethod(
  f = "convert",
  signature = c(target = "NmModel2", source = "ANY", component = "PrmLogitNormal"),
  definition = function(target, source, component, options) {
    values <- parameter_values(component)
    target <- target +
      nm_theta_record(component@name, initial = values["mean_logit"], lbound = 0, ubound = 1) +
      nm_omega_record(component@name, initial = values["var_logit"])
...
setMethod(
  f = "convert",
  signature = c(target = "NmModel2", source = "ANY", component = "PrmNoVar"),
  definition = function(target, source, component, options) {
    values <- parameter_values(component)
    target <- target +
      nm_theta_record(component@name, initial = values["value"], lbound = 0)
