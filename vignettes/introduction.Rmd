...
```{r}
m <- model()+
  prm_log_normal("EMAX", values = c(log(12),0.1))+
  prm_log_normal("ED50", values = c(log(12),0.09))+
...
m <- model()+
  ...
  prm_log_normal("cl", values = c(log_mu = 4.5, log_sigma = 0.1))+
  prm_log_normal("vc", values = c(log_mu = 1, log_sigma = 0.1))+
  obs_additive(conc~C["central"], values = c(sigma = 1))
