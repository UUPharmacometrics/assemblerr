...
Let's start simple and assume you want to build a dose-response model using the Emax function. The following code would accomplish that:
...
The first line is the so-called constructor, something like the foundation of what you are trying to build. In this example, we are building a general pharmacometric model. Later, we will discuss when and why you would choose something else. All other lines specify building blocks and combine them using the `+` operator.  What each of the building blocks defines is hopefully clear from its name, but let's go through them anyway to be sure:  
  ...
  - Line 4 defines the an algebraic relationship for the variable effect using a classical `R` formula. In these formulae, the `~` should be understood like a `=`. You can use formulae in assemblerr whenever you want to define a mathematical relationship.   
  - Line 5 defines the observational model, i.e., what to observed (the variable "effect" in this case) as well as the model for residual error (additive) and the variance of the error term.
  
Let's assume you want to create a model with a proportional error model instead, you simply would need to exchange the last line to something like `obs_proportional(~effect, var_prop = 0.05)`. Similar, if you would want to switch one of the parameter models. The use of these exchangable building blocks is the core principle in assemblerr. 
...
The `render` function also has an options argument that allows you to modify some aspects of the rendering process. For example, the following code automatically adds mu-referencing code for all parameters.
...
"All models are wrong" AND useless if we can not do anything with them. So far, you only saw the description of a model. The `tasks=` argument of the `render()` function allows to specify what the model should "do".  
...
Many pharmacometric models are defined using differential equations. In assemblerr you can specify differential equations via compartments and flows (while this does not cover all types of differential equations, it will hopefully get you pretty far). To show compartment and flows in action, we will extend the dose-response model from the beginning to a concentration-response model. The following code chunk shows the necessary changes:  
...
In this model definition, we used two additional parameters and, more importantly, a compartment as well as a flow from that compartment. You should note, how you can use the variable `C` in the flow definition to refer to the concentration in the "from" compartment. Similarily, you can also use `C["central"]` to refer to the concentration in the central compartment when defining the observational model. The `A` variable provides a reference to the unscaled amount in the compartment. 
...
Despite its slighly different nature, you can turn the model into NONMEM code using the `render()` function. Also, tasks and rendering options work like in the previous examples. So you could do the following to create NONMEM code with mu-referencing, importance sampling estimation, and a table containing all model parameters: 
...
Alternativly, you can overwrite the default parameter model by adding a another one with the same name, i.e., :
...
There is more to learn about assemblerr and you have multiple sources available to do so:  

  - Help for a specific function can be found in the reference documentation in R that can be accessed using `?`. For example, `?prm_log_normal` will open the documentation for the log-normal parameter building block.
  - An overview of the available documentation is available via `help(package="assemblerr")`.
  - The vignette building blocks provides an overview of all the building blocks available in assemblerr.
